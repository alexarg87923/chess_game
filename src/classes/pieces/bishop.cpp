...
std::vector<std::queue<std::shared_ptr<Hitbox>>> Bishop::calc_moves(const Position& pos) {
    ...
    for (char i = row_start - 1, j = col_start + 1; i >= 'A' && j <= col_end; i--, j++) {
        Position pos{i, j};
        up_right.push(std::make_shared<Hitbox>(pos, COORDINATES[pos], self));
    }
    ...
    for (char i = row_start + 1, j = col_start + 1; i <= row_end && j <= col_end; i++, j++) {
        Position pos{i, j};
        down_right.push(std::make_shared<Hitbox>(pos, COORDINATES[pos], self));
    }
    ...
    for (char i = row_start + 1, j = col_start - 1; i <= row_end && j >= 1; i++, j--) {
        Position pos{i, j};
        down_left.push(std::make_shared<Hitbox>(pos, COORDINATES[pos], self));
    }
    ...
    for (char i = row_start - 1, j = col_start - 1; i >= 'A' && j >= 1; i--, j--) {
        Position pos{i, j};
        up_left.push(std::make_shared<Hitbox>(pos, COORDINATES[pos], self));
    }
